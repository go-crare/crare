...
// used by Crare in sending and editing media methods.
...
// Instead, Crare chooses the hi-res one and just sticks to it.
//
...
func (p *Photo) UnmarshalJSON(data []byte) error {
	...
	if data[0] == '{' {
		if err := defaultJson.Unmarshal(data, &hq); err != nil {
			return err
		}
	} else {
		var sizes []photoSize
		if err := defaultJson.Unmarshal(data, &sizes); err != nil {
			return err
		}
