...
type Settings struct {
	...
	Webhook    *crare.Webhook    `yaml:"webhook"`
	LongPoller *crare.LongPoller `yaml:"long_poller"`
}
	...
	for _, item := range aux.Buttons {
		...
		if !btn.IsReply && btn.Data != nil {
			if a, ok := btn.Data.([]any); ok {
				...
				return fmt.Errorf("crare/layout: invalid callback_data for %s button", k)
			}
		}
		...
		if yaml.Unmarshal(data, &shortenedMarkup) == nil {
			...
			for i, btns := range shortenedMarkup {
				...
				for j, btn := range btns {
					b, ok := lt.buttons[btn]
					if !ok {
						return fmt.Errorf("crare/layout: no %s button for %s markup", btn, k)
					}
					row[j] = b
				}
				kb[i] = row
			...
			for _, row := range kb {
				for _, btn := range row {
					...
					if markup.inline == nil {
						...
						return fmt.Errorf("crare/layout: mixed reply and inline buttons in %s markup", k)
					}
				}
			}
			...
			kb := make([][]crare.ReplyButton, len(markup.Keyboard))
			for i, btns := range markup.Keyboard {
				row := make([]crare.ReplyButton, len(btns))
				for j, btn := range btns {
