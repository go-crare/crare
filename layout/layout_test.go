...
func TestLayout(t *testing.T) {
	...
	assert.Equal(t, &crare.LongPoller{}, pref.Poller)

	assert.ElementsMatch(t, []crare.Command{{
		Text:        "start",
	...
	assert.Equal(t, crare.ChatID(123), lt.ChatID("num"))
	...
	assert.Equal(t, &crare.ReplyMarkup{
		ReplyKeyboard: [][]crare.ReplyButton{
			{{Text: "Help"}},
			{{Text: "Settings"}},
		},
		ResizeKeyboard: true,
	...
	assert.Equal(t, &crare.ReplyMarkup{
		ReplyKeyboard:   [][]crare.ReplyButton{{{Text: "Send a contact", Contact: true}}},
		ResizeKeyboard:  true,
		OneTimeKeyboard: true,
	}, lt.MarkupLocale("en", "reply_extended"))

	assert.Equal(t, &crare.ReplyMarkup{
		InlineKeyboard: [][]crare.InlineButton{{
			{
				Unique: "stop",
				Text:   "Stop",
				Data:   "1",
		...
		InlineKeyboard: [][]crare.InlineButton{{
			{
				...
				WebApp: &crare.WebApp{URL: "https://google.com"},
			},
		}},
	}, lt.MarkupLocale("en", "web_app"))

	assert.Equal(t, &crare.ArticleResult{
		ResultBase: crare.ResultBase{
			ID:   "1853",
			Type: "article",
		},
		Title:       "Some title",
