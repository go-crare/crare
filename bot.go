...
type Settings struct {
	...
	// see: https://pkg.go.dev/gopkg.in/crare/pkg/json.
	//
...
//	})
//
//	b.Handle(&inlineButton, func (c *crare.Context) error {
//		return c.Respond(&crare.CallbackResponse{Text: "Hello!"})
//	})
//
// Middleware usage:
//
...
func (b *Bot) Handle(endpoint any, h HandlerFunc, m ...HandlerFunc) {
	...
	default:
		b.logger.Panicf("crare: unsupported endpoint")
	}
}
...
func (b *Bot) Start() {
	if b.Poller == nil {
		b.logger.Panicf("crare: can't start without a poller")
	}
...
//	b.Edit(m, "new <b>text</b>", crare.ModeHTML)
//	b.Edit(m, &crare.ReplyMarkup{...})
//	b.Edit(m, &crare.Photo{File: ...})
//	b.Edit(m, crare.Location{42.1337, 69.4242})
...
// Use cases:
//
//	b.EditMedia(m, &crare.Photo{File: crare.FromDisk("chicken.jpg")})
//	b.EditMedia(m, &crare.Video{File: crare.FromURL("http://video.mp4")})
...
func (b *Bot) File(file *File) (io.ReadWriteCloser, error) {
	...
	if !resp.IsStatusCode(200) {
		return nil, fmt.Errorf("crare: expected status 200 but got %v", resp.StatusCode())
	}
	return w, nil
}
